...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    ...
    if AI_jogando:
        texto = FONTE_PONTOS.render(f"Aprendizagem: {geracao}", 1, (255, 255, 255))
        tela.blit(texto, (TELA_LARGURA - 1 - texto.get_width(), 10))
    chao.desenhar(tela)
...
def tela_game_over(tela):
    if not AI_jogando:
        rodando = True
        while rodando:
            tela.blit(IMAGEM_TELA_GAME_OVER, (0, 0))
            pygame.display.update()
